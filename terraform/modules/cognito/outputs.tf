...
output "cognito_user_pool_client_id" {
  ...
  value       = aws_cognito_user_pool_client.videocoreapp_cognito_client.id
}

output "cognito_code_login_url" {
  description = "URL de login do Cognito User Pool (usando o fluxo de authorization code)"
  value = "https://${aws_cognito_user_pool_domain.cognito_user_pool_domain.domain}.auth.${var.aws_location}.amazoncognito.com/login?client_id=${aws_cognito_user_pool_client.videocoreapp_cognito_client.id}&response_type=code&scope=email+openid&redirect_uri=${var.callback_urls[0]}"
}
...
output "cognito_implicit_login_url" {
  ...
  value = "https://${aws_cognito_user_pool_domain.cognito_user_pool_domain.domain}.auth.${var.aws_location}.amazoncognito.com/login?client_id=${aws_cognito_user_pool_client.videocoreapp_cognito_client.id}&response_type=token&scope=email+openid&redirect_uri=${var.callback_urls[0]}"
}